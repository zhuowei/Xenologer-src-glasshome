  ...
  implements Iterator<PhoneNumberMatch>
{
  ...
  static boolean allNumberGroupsRemainGrouped(PhoneNumberUtil paramPhoneNumberUtil, Phonenumber.PhoneNumber paramPhoneNumber, StringBuilder paramStringBuilder, String[] paramArrayOfString)
  {
    ...
    for (int j = 0; j < paramArrayOfString.length; j++)
    {
      ...
      if ((j == 0) && (i < paramStringBuilder.length()) && (Character.isDigit(paramStringBuilder.charAt(i))))
      {
  ...
  static boolean containsMoreThanOneSlash(String paramString)
  {
    int i = paramString.indexOf('/');
    return (i > 0) && (paramString.substring(i + 1).contains("/"));
  }
  ...
  static boolean isNationalPrefixPresentIfRequired(Phonenumber.PhoneNumber paramPhoneNumber, PhoneNumberUtil paramPhoneNumberUtil)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        localNumberFormat = paramPhoneNumberUtil.chooseFormattingPatternForNumber(localPhoneMetadata.numberFormats(), str1);
      }
      while ((localNumberFormat == null) || (localNumberFormat.getNationalPrefixFormattingRule().length() <= 0) || (localNumberFormat.isNationalPrefixOptionalWhenFormatting()));
      str2 = localNumberFormat.getNationalPrefixFormattingRule();
    }
    while (PhoneNumberUtil.normalizeDigitsOnly(str2.substring(0, str2.indexOf("$1"))).length() == 0);
