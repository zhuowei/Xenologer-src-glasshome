...
public class ItemScroller extends SimpleInputListener
{
  ...
  public ItemScroller(Context paramContext, final Scrollable paramScrollable)
  {
    ...
    this.flingAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
    {
      public void onAnimationUpdate(ValueAnimator paramAnonymousValueAnimator)
      {
        ...
        label285: for (int j = 1; ; j = 0)
        {
          ...
          if (i != 0)
          {
            ItemScroller.this.logVerbose("hitEdge: " + f4);
  ...
  private boolean snapToNearestInDirection(float paramFloat)
  {
    ...
    if ((this.scrollable.getScale() == 1.0F) && (this.animationLeftover != 0.0F))
    {
      ...
      logVerbose("Confirm while moving one item at a time. Snapping to: " + f);
  ...
  {
    logVerbose("startFling with velocity: " + paramFloat);
    stopFling();
  ...
  public boolean onFingerCountChanged(int paramInt, boolean paramBoolean)
  {
    ...
    do
      while (true)
      {
        ...
        if (paramInt > 1)
        {
          logVerbose(paramInt + " fingers down");
          continue;
          if (paramInt == 2)
          {
    ...
    {
      bool = true;
      label178: this.zoomedOutOnUp = bool;
  ...
  public boolean onPrepareSwipe(int paramInt1, float paramFloat1, float paramFloat2, float paramFloat3, float paramFloat4, int paramInt2, int paramInt3)
  {
    ...
    while (true)
    {
      ...
      {
        f1 = paramFloat1 - this.lastAccumulatorX;
      ...
      return true;
      label126: if (Math.abs(paramFloat1) > Math.abs(paramFloat2))
  ...
  {
    logVerbose("onSwipe: " + paramInt + ", " + paramSwipeDirection);
  ...
  {
    logVerbose("Hard setting to: " + paramInt);
    stopFling();
